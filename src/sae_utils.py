...
import warnings
import math
from src.base_sae import SparseAutoencoder, TopKSparseAutoencoder, BatchTopKSparseAutoencoder
...
def train_sae_fast(
    ...
    *,
    ...
    X = latent_tensor.float().to(device)
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, N, batch_size):
            ...
            if z_l2_lambda > 0:
                loss = loss + z_l2_lambda * z.pow(2).mean()
            ...
            # GPU accumulation; no .item() per batch
            running_total += loss.detach() * bsz
            running_recon += recon_loss.detach() * bsz
            l0_sum += (z > 0).sum().detach()

        if verbose_every and (
            epoch == 0 or (epoch + 1) % verbose_every == 0 or epoch == epochs - 1
        ):
            avg_total = (running_total / N).item()
            avg_recon = (running_recon / N).item()
            avg_l0 = (l0_sum / N).item()

            print(
                f"Epoch {epoch+1:3d}/{epochs} | "
                f"total {avg_total:.10f}  recon {avg_recon:.10f}  L0 {avg_l0:.3f}"
            )
...
def train_sae(
    ...
    *,
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=False, pin_memory=True, num_workers = 16)
...
def _topk_similarity_matrix(
    ...
    **kwargs,
    ...
    """
    Fast top-k overlap/Jaccard similarity between concept activation matrices.

    acts_a: [N, Da]
    acts_b: [N, Db]

    Returns:
        [Da, Db] similarity matrix
    """
    ...
    membership_dtype = torch.float16 if device.startswith("cuda") and k_eff <= 2048 else torch.float32

    def make_membership(x: torch.Tensor):
        """
        x: [N, D]
        returns:
            mem:   [N, D] binary top-k membership matrix
            sizes: [D] number of selected active samples per concept
        """

        # Top-k over samples for every concept at once
        vals, idx = torch.topk(
            x,
            k=k_eff,
            dim=0,
        ...
        mem.scatter_(0, idx, valid.to(membership_dtype))

        sizes = valid.sum(dim=0).to(torch.float32)

        return mem, sizes

    mem_a, sizes_a = make_membership(a)
    mem_b, sizes_b = make_membership(b)
    ...
    sizes_b = sizes_b[None, :]

    if metric == "jaccard":
        denom = sizes_a + sizes_b - inter
    else:
        denom = torch.minimum(sizes_a, sizes_b)
...
def _rank_transform_cols(x, device = None):
    return rankdata(x, method="average", axis=0)


@torch.no_grad()
def _rank_transform_cols_fast(x: torch.Tensor, device = None) -> torch.Tensor:
    """
    Fast column-wise rank transform on GPU.

    Equivalent to ordinal ranks per column.
    For continuous activations, this is usually close to scipy rankdata(..., method="average"),
    because exact ties are rare.
    """
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    ranks.scatter_(0, order, rank_values.expand(n, d))
    return ranks

import torch

@torch.no_grad()
def rank_transform_cols_torch_average_ties(
    x,
    *,
    device=None,
    ...
    """
    Column-wise rank transform with average tie handling.

    Equivalent in spirit to:
        scipy.stats.rankdata(x, method="average", axis=0)

    Input:
        x: [N, D]

    Output:
        ranks: [N, D], float32, 1-based average ranks
    """
    if x.ndim != 2:
    ...
    sorted_x = torch.gather(x, 0, order)

    # 1-based rank positions in sorted order
    pos = torch.arange(
        1, n + 1,
        device=x.device,
        dtype=torch.float32,
    ...
    # Mark start of each tie group
    new_group = torch.ones((n, d), device=x.device, dtype=torch.bool)
    new_group[1:] = sorted_x[1:] != sorted_x[:-1]

    # Group ids within each column
    group_id = torch.cumsum(new_group.to(torch.long), dim=0) - 1

    # Make group ids globally unique across columns
    num_groups_per_col = group_id[-1] + 1

    offsets = torch.cumsum(
        torch.cat([
            torch.zeros(1, device=x.device, dtype=torch.long),
            num_groups_per_col[:-1],
        ]),
        dim=0,
    )

    global_group_id = group_id + offsets[None, :]

    total_groups = int((offsets[-1] + num_groups_per_col[-1]).item())

    flat_gid = global_group_id.reshape(-1)
    flat_pos = pos.reshape(-1)

    counts = torch.bincount(flat_gid, minlength=total_groups).to(torch.float32)
    sums = torch.bincount(flat_gid, weights=flat_pos, minlength=total_groups)

    avg_rank_per_group = sums / counts.clamp_min(1.0)

    sorted_ranks = avg_rank_per_group[global_group_id]

    ranks = torch.empty((n, d), device=x.device, dtype=torch.float32)
...
def _cos_similarity_matrix_1(
    ...
    **kwargs,
    ...
    """
    Fast centered cosine similarity between columns.
    ...
    if a.ndim != 2 or b.ndim != 2:
        raise ValueError("a and b must be 2D arrays/tensors [N, D].")

    if a.shape[0] != b.shape[0]:
        raise ValueError(f"a and b must have same N. Got {a.shape[0]} and {b.shape[0]}.")

    # Use float32 for mean/norm for stability
    a = a.float()
    b = b.float()
    ...
    # Cast only for GEMM/output speed
    a = a.to(dtype)
    b = b.to(dtype)

    sim = a.T @ b
    sim = sim.to(torch.float16)
...
def _cos_similarity_matrix(
    ...
    *,
    ...
    # Always use float32 for centering/norms
    a = a.float()
    b = b.float()
...
@torch.no_grad()
def _get_random_similarity_matrix_torch(
    a,
    b,
    method: str = "cos",
    N: int = 10,
    ...
    *,
    ...
    """
    Random baseline for centered cosine similarity.
    ...
    For each repeat:
        ...
        compute cosine similarity against b
        take either diagonal or max over a-concepts

    Returns:
        [Db] if fixed_sae=False
        [D]  if fixed_sae=True and Da == Db
    """

    if method != "cos":
        raise ValueError("Only method='cos' is implemented.")
    ...
    # Center + normalize columns once.
    # Row permutation does not change column means/norms.
    a = a - a.mean(dim=0, keepdim=True)
    b = b - b.mean(dim=0, keepdim=True)
    ...
    # CPU float16 matmul is usually bad; GPU float16 can be useful.
    if matmul_dtype is None:
        matmul_dtype = torch.float16 if device.type == "cuda" else torch.float32

    if device.type == "cuda":
        a_mm = a.to(matmul_dtype)
        b_mm = b.to(matmul_dtype)
    else:
        a_mm = a
        b_mm = b

    out_dim = db
    acc = torch.zeros(out_dim, device=device, dtype=torch.float32)
    ...
    for _ in range(repeats):
        ...
        # Equivalent to cosine(a[perm], b), but normalization is already done.
        sim = a_mm[perm].T @ b_mm
        sim = sim.to(torch.float32)

        if fixed_sae:
            acc += torch.diagonal(sim)
        else:
            acc += sim.max(dim=0).values
...
def _flatten_features(x: torch.Tensor) -> torch.Tensor:
    ...
    """
    if x.ndim < 2:
        raise ValueError(f"Expected tensor with shape [N, ...], got {tuple(x.shape)}")
...
def _center_columns(x: torch.Tensor) -> torch.Tensor:
    ...
    If x has shape [N, D], this subtracts the mean of each feature column.
    """
...
def linear_cka(
    ...
    Args:
        ...
        center: Whether to mean-center features across samples

    Returns:
        Scalar tensor with CKA similarity in [0, 1] approximately
        (up to tiny numerical error).
    """
    ...
    hsic = (x_ty ** 2).sum()
    norm_x = torch.linalg.norm(x_tx, ord="fro")
    norm_y = torch.linalg.norm(y_ty, ord="fro")
    cka = hsic / (norm_x * norm_y + eps)
    return _to_numpy(cka)
...
def get_saes_similarities(
    ...
    **kwargs: Any,
    ...
    #start = time.time()
    if return_cos:
        sim_cos = _cos_similarity_matrix(acts_a, acts_b)
    else:
        sim_cos = np.zeros((acts_a.shape[1],acts_a.shape[1]))

    if get_random:
        random_cos = _get_random_similarity_matrix_torch(acts_a, acts_b, method="cos", N=25, fixed_sae=False)
    else:
        random_cos = np.zeros_like(sim_cos).diagonal(0)
    #cos = time.time()
    sim_topk = _topk_similarity_matrix(acts_a, acts_b, metric=metric, **kwargs)
    #top = time.time()
    if method == "spearman":
        if device == "cuda":
            a_use = rank_transform_cols_torch_average_ties(acts_a,device=device)
            b_use = rank_transform_cols_torch_average_ties(acts_b,device=device)           
        else:
            a_use = _rank_transform_cols(acts_a)
            b_use = _rank_transform_cols(acts_b)
            a_use = torch.as_tensor(a_use, device=device)
            b_use = torch.as_tensor(b_use, device=device)
    ...
    sim_corr = _cos_similarity_matrix(a_use, b_use)
    if get_random:
        random_corr = _get_random_similarity_matrix_torch(a_use, b_use, method="cos", N=25, fixed_sae=False)
    else:
        random_corr = np.zeros_like(sim_corr).diagonal(0)
    #corr = time.time()
    #print(f"cos {cos - start} top {top-cos} rank {rank-top} cor {corr-rank}")
